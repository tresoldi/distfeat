...
_IPA_REVERSE: dict[str, str] = {value: key for key, value in _IPA_EQUIVALENCES.items()}
...
def build_class_table(dataset: FeatureDataset) -> dict[str, frozenset[str]]:
    """Build SOUND_CLASS -> feature set from the dataset."""
    result: dict[str, frozenset[str]] = {}
    for class_name, feat_str in dataset.class_features.items():
        if feat_str:
            features = frozenset(value.strip() for value in feat_str.split(",") if value.strip())
            if features:
...
class IPAFeatureSystem:
    ...
    def matches(self, pattern: object, target: object) -> bool:
        if not isinstance(pattern, CategoricalFeatures) or not isinstance(target, CategoricalFeatures):
            msg = "IPA matching requires CategoricalFeatures inputs."
    ...
    def segment_distance(self, a: object, b: object) -> float:
        if not isinstance(a, CategoricalFeatures) or not isinstance(b, CategoricalFeatures):
            msg = "IPA segment_distance requires CategoricalFeatures inputs."
            raise NotImplementedError(msg)
